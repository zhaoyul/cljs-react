...
本章中的示例位于 ~first-project.atoms~ 命名空间. 我们建议您创建此文件并跟随操作！
...
如果您内联评估一个原子, 您会看到它的初始值和包裹对象. 这可能会根据底层语言而有所不同. 对于JavaScript, 它看起来像：


#+INCLUDE: "./protected/source_code/first-project/src/first_project/atoms.cljs" src clojure :lines "11-12"
** 获取原子的当前值
Clojure原子是Clojure引用（reference）的一个实例. 我们不会深入讨论refs. 但您可以将ref视为原子的基类.
...
** 重置当前值
如果您想在不关心初始值的情况下更新值, 可以使用 ~reset!~ 函数. 这个函数接受两个参数, 要重置的原子及其新值.
...
您还记得您在 ~语法和原生数据类型~ 章节中首次遇到这些函数的情况吗？您还记得这些操作的不可变性吗？

 ~(assoc {} :a :b)~ 不会更新原始映射, 而是返回一个新的. 原子帮助我们安全地变更状态.

** 观察变化
您可以添加一个钩子来观察对原子状态所做的更改. 在您希望在状态改变后运行不纯的操作的情况下, 这很有用. 例如, 您可能想要在原子更新时记录更改或重新渲染UI.
...
![打印的日志语句](./public/assets/log-statement.png)
...
** 使用 ~let~ 绑定的局部原子
到目前为止, 我们使用的所有原子都是使用 ~def~ 宏定义的. 使用 ~def~ 生成的变量在那个命名空间中是全局的.  ~let~ 函数允许您定义局部变量：
