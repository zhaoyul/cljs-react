...
介绍clojure作为一门lisp语言的一些特性和c系列语言的差异比较. 介绍如何clojurescript是一门更好的js语言.

着眼前端工具链, 适合有一定基础的React开发者的, 在这个教程中, 我们将构建一个的SVG canvas图形编辑器. 为接下来的全功能的svg编辑器做知识储备.
...
- [X] Pareditl(结构化编辑的工具. 非常轻松地编写和修改S-表达式, 以至于括号几乎会消失.)
*** clojure的标准库
