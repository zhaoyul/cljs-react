...
Paredit帮助进行结构化编辑. 用非术语来说, 它是一个辅助编写S-表达式的工具. 由于Paredit依赖于使用的编辑器, 我们将在本章中只覆盖核心特性.
...
我们在 ~Getting Started~ 部分安装了Paredit. VSCode不需要单独安装Paredit, 因为它随Calva一起提供.
...
** 结论
在这一章中, 我们学习了Paredit和结构化编辑. 很有可能会觉得它很烦人.
Paredit是所有Lisper最终都会欣赏的工具. 只需给它一些时间, 让它沉淀.
一旦理解了基础知识, 代码将感觉更简洁, 更易于编辑.

假设一个人可以一次性学会所有这些命令是不合理的, 事实是不需要.
将本章作为参考, 需要时再回来查看. 随着时间的推移, 这些命令将进入肌肉记忆.
我们建议先关注Slurp和Barf. 我们还建议你重新绑定你的快捷键到emacs的方式, 使Paredit更符合人体工程学.
