...
#+DESCRIPTION: 我们的第一个项目现在已经就位. 在这一章中，我们将学习如何构建并运行它. 我们还将探索一些其他构建目标，并使用Reagent（一个ClojureScript包装器）探索Shadow和React.
#+SETUPFILE: https://gitee.com/zhaoyuKevin/org-theme/raw/master/org/theme-readtheorg.setup

Shadow提供了许多助手来编译和开发你的应用, 但我们现在只需要了解三种方法：
- ~Compile~
- ~Watch~
- ~Release~
...
#+begin_quote
当你第一次运行 ~watch~ 命令时, Shadow将安装所需的依赖.
#+end_quote
...
如果一切顺利, 你应该看到一些日志声明构建完成并且NREPL服务器正在运行：
...
** 运行脚本
此时, 你的ClojureScript代码被编译成了 ~build/node-script/core.js~ 中的Node脚本. 你可以检查这个代码, 因为它是JavaScript, 但它可能没有多大意义. ~:node-script~ 目标将所有内容捆绑到一个文件中, 这使得在可读性上的代价下易于分发.

然而, 我们可以简单地运行编译后的Node脚本：
...
你可能会注意到进程没有终止. 这是因为Shadow在 ~watch~ 模式下允许一些钩子, 每次你做出更改时都可以在Node环境中执行你的代码. 现在, 你可以简单地按 ~Ctrl+C~ 或 ~Cmd+C~ 退出.

尝试将 ~core.cljs~ 中的消息更改为"Hello World 2". 你应该注意到, 当你保存该文件时, Shadow会重新构建脚本.
...
如果你再次在Node中运行脚本, 进程将终止, 因为脚本是为生产使用打包的：
...
## 设置发布和观察快捷方式
有时, 键入整个构建命令可能感觉繁琐. 我的做法是将这些命令添加到NPM脚本中, 这样它们就更容易使用了.
...
现在, 我们可以使用 ~yarn w:script~ 而不是 ~yarn shadow-cljs watch :script~. 随意尝试适合你的别名.

** 自动化脚手架
使用Shadow设置一个新的ClojureScript项目很简单, 但不容易. 你需要创建配置, 目录结构, 并设置一些构建目标.

[[https://github.com/filipesilva/create-cljs-app][create-cljs-app]]可以帮助简化这个过程. 这个工具在范围上类似于 ~create-react-app~.

要使用该工具创建一个新项目, 你可以这样做：
...
这将为你创建一个Shadow配置, 一个NPM配置, 安装依赖, 并为你的代码和测试设置一个目录结构. 它还将安装一个linter并为运行, 打包和测试你的应用程序设置NPM脚本别名.
...
一旦你理解了生态系统并希望快速引导项目, 这是一个很好的开始方式.

** 针对浏览器

使用 ~create-cljs-app~ 生成的应用程序是一个在浏览器中运行的React应用程序. 它在底层使用了[[https://reagent-project.github.io/][Reagent]]并设置了一个基本的SPA. 如果你在新创建的 ~second-project~ 目录中运行 ~yarn start~, Shadow将为你启动一个端口为3000的开发服务器和一个端口为3333的nREPL(假设你没有更改任何配置).

如果你检查 ~http://localhost:3000~, 你会看到一个简单的React组件. 以下配置负责编译应用程序以在浏览器中运行：
...
例如, 你的Web服务器被配置为从`/root/example-com/public`目录在https://example.com上提供静态网站. 并且你将代码编译到`public/js`目录, 那么你的资产路径相对于Web服务器是`/js`, 即如果一个名为`public/index.html`的文件需要包含一个编译后的脚本, 那么它可以在`http://example.com/js/main.js`(或简单地`/js/main.js`)中这样做.
...
随着你的应用程序规模的增长, 你可以将输出分割成多个模块. 模块也可以相互依赖. Shadow将计算依赖图, 并尝试将大多数代码推到边缘.

通常的做法是有一个 ~:shared~ 模块来加载基本依赖, 一个 ~:main~ 模块来初始化应用程序, 以及多个特定于领域的模块, 如 ~:login~, ~:dashboard~ 等.

模块配置将随着我们使用它而变得更容易掌握. 现在, 只需将其视为将你的代码放入不同篮子中的选项.

*** 在index.html中包含main
基于浏览器的JavaScript需要一些HTML. 它可能不是从配置中明显, 但如果你检查 ~create-cljs-app~ 生成的 ~public~ 文件夹, 你会发现一个 ~index.html~ 文件.
...
- 你开始使用 ~yarn start~ 开始watch ~:app~ 这个目标
- Shadow将CLJS代码编译为JavaScript并输出到 ~public/js~ 目录
- 开发服务器在端口3000上提供这个目录
- 你打开浏览器检查输出
- 当你做出更改时, Shadow会重新编译你的应用程序
- 它还会热重载！
