...
你可能会想, 刚刚在上一课中学习了Clojure的语法, 现在却说Lisps没有语法!?. 我们知道我们的说法听起来有些奇怪. 但是到本课结束时, 它会变得有意义.

在C家族语言如JavaScript中, 下面的条件判断司空见惯:
...
当然, C风格版本可以使用三元运算符重写, 很好! 你在正确的思考轨道上. C风格版本可以重写为:
...
三元运算符对于上述简单场景来说足够好, 但随着我们的应用变得更加复杂, 三元运算符变得难以管理. 新的语法字面量会不断增加程序员的心智负担.

而且引入新的语法阻碍了向后兼容性. 然而, Lisps的语法是语言本身定义的概念, 所以没有语法兼容性的问题.

由于大多数著名的语言都是C风格的, 所以语法的概念在每个人的脑海中都根深蒂固. 这导致的代码不如Lisp那样具有表达性.

Lisp并不总是比C风格语言更具表达性. 只是Lisps通过不强制执行语法, 使开发人员能够编写更简洁的代码. 随着我们的程序变得更复杂, 这一属性将变得更加明显.

** 重新定义核心
在Clojure中, 可以使用 ~defn~ 函数重新定义 ~if~ 来表示别的东西:
