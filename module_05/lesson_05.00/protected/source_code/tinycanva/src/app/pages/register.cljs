  ...
  (let [{:keys [email password]} @state] ; 1
    ...
    [:div.w-full.h-screen.flex.bg-gray-400 ; 2
     [:div {:class "w-2/3 md:w-1/3 m-auto bg-white px-4 rounded shadow"} ; 3
      ...
      [:div.my-4.text-2xl "Register to Tinycanva"]
      ...
      ;; 5
      [:> Button {:text "Register" :icon "log-in" :intent (.-PRIMARY Intent)
                  :class "mt-4 mb-2" :loading false
                  :onClick #()
                  }]
      ;; 6
      [:div.my-8.bg-gray
       "Already have an account? " [:> Link {:to "/login" :class "text-sm text-gray-600"} "Login"]]
